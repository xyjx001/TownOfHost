    ...
    class ChanceChangePatch
    {
        public static void Postfix(RoleOptionSetting __instance)
        {
            ...
            if (__instance.Role.Role == RoleTypes.Shapeshifter)
            {
                ...
                if (CustomRoles.SerialKiller.IsEnable() || CustomRoles.Warlock.IsEnable() || CustomRoles.BountyHunter.IsEnable() || (CustomRoles.SPImpostor.IsEnable() && SPImpostor.RoleType.GetSelection() == 1)/* || CustomRoles.ShapeMaster.IsEnable()*/) forced = true;
            }
